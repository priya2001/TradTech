...
export const signup = catchAsync(async (req, res, next) => {
    const { name, shopName, email, password, passwordConfirm, mobileNumber, licenseNumber, address } = req.body;

    // Basic validation
    if (!licenseNumber) {
        return next(new AppError('License number is required for registration', 400));
    }
    ...
    const newShopkeeper = await Shopkeeper.create({
        ...
        mobileNumber,
        licenseNumber,
        address: {  // Make sure to include the address object
            location: {
                type: 'Point',
...
export const protect = catchAsync(async (req, res, next) => {
    ...
    if (!currentShopkeeper) {
        return next(
            new AppError('The shopkeeper belonging to this token no longer exists.', 401)
        );
    }
    ...
    if (currentShopkeeper.changedPasswordAfter(decoded.iat)) {
        return next(
            new AppError('Shopkeeper recently changed password! Please log in again.', 401)
        );
    }
...
export const updatePassword = catchAsync(async (req, res, next) => {
    ...
    if (!(await shopkeeper.correctPassword(req.body.currentPassword, shopkeeper.password))) {
